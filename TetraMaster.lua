...
local function log_party_ids()
    ...
    lines[#lines + 1] = 'Duel sessions (this client):'
    for _, slot in ipairs(PARTY_MEMBER_SLOTS) do
        local member = party and party[slot]
...
local function accept_challenge()
    ...
    if not session_id then
        chat('could not resolve character IDs for the duel.')
        chat('Make sure you are still in party.')
        return
    end
...
local function handle_command(command, ...)
    ...
    elseif command == 'debug' then
        ...
        if debug_mode then
            chat('debug mode on. //tm pid writes diagnostics to data/pid.log (not chat).')
        else
            chat('debug mode off.')
        end
    elseif command == 'pid' then
        ...
        log_party_ids()
        chat('party details written to data/pid.log')
