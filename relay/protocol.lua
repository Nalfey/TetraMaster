...
local function split_handshake_body(body, delimiter)
  local parts = {}
  for part in body:gmatch("[^" .. delimiter .. "]+") do
    parts[#parts + 1] = part
  end
  return parts
end
...
function protocol.parse_handshake(text)
  ...
  if text:sub(1, #protocol.TM_PREFIX) == protocol.TM_PREFIX then
    offset = #protocol.TM_PREFIX + 1
    parts = split_handshake_body(text:sub(offset), "|")
  elseif text:sub(1, #protocol.TM_LEGACY_PREFIX) == protocol.TM_LEGACY_PREFIX then
    offset = #protocol.TM_LEGACY_PREFIX + 1
    parts = split_handshake_body(text:sub(offset), "/")
  else
    return nil
  end
...
function protocol.parse_friendly_accept(text)
  if not text then
    return nil
  end

  text = text:gsub("^%b() %s*", "")
  return text:match("^([%w]+) accepts the duel!$")
end

function protocol.parse_friendly_decline(text)
  if not text then
    return nil
  end
